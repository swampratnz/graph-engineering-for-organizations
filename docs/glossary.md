...
- **GRAPH SPEC** — the one-file contract for a workflow: frontmatter
  (machine-validated) + prose. Template: [`specs/TEMPLATE.md`](../specs/TEMPLATE.md).
...
- **Exception** — the only sanctioned way to run out of compliance with a
  validator rule: named approver + expiry ≤ 90 days, checked by CI
...
- **Fresh-context verifier** — an agent that re-checks a judgment with
  ...
  confidence cascades.
- **Idempotency key** — `(run_id, step_id)`-derived key making any
