...
| `GE-AGENT-NOKILL` | Agent entry has no kill switch | Add `kill_switch.how` + named `authorized` holders |
...
| Code | Trips when | Fix |
|------|-----------|-----|
| `GE-EXC-INVALID` | An entry is malformed: missing fields, bad dates, duplicate id, empty approver list, or expiry > 90 days from grant | Complete the entry per the template comments in [`exceptions.yaml`](../governance/exceptions.yaml) |
