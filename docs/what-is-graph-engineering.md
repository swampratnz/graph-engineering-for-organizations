...
contracts in the graph, declared in versioned files, and enforced by CI.**

Concretely, a graph-engineered workflow is defined by its artifacts:

- **Typed nodes**: agent steps, deterministic steps, and — first-class, not
  bolted on — *human gates* with contracts (structured input, an
  approve/reject/modify + reason output, and explicit timeout behavior).
...
- **Versioned governance**: who owns the workflow, what identity it runs
  as, what it may spend, how it's stopped — written down as files
...
required here**. The graphs in this repo are workflow topologies, not data
models.
...
(July 2025; since labeled historical — the perception gap is the durable
lesson). [DORA 2025](https://dora.dev/dora-report-2025/) finds AI raises
...
dishonestly relax the measurement — which is why the instruments that
...
requires oversight with the ability to interrupt to a safe state — a
[kill switch](runbooks/kill-switch.md) with named holders.
